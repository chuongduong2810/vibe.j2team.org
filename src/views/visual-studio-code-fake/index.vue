...
const tabs: FileTab[] = [
  { id: 'html', label: 'index.html', ext: 'HTML', colorClass: 'text-accent-coral', bgClass: 'bg-accent-coral' },
  { id: 'css', label: 'style.css', ext: 'CSS', colorClass: 'text-accent-sky', bgClass: 'bg-accent-sky' },
  { id: 'js', label: 'script.js', ext: 'JS', colorClass: 'text-accent-amber', bgClass: 'bg-accent-amber' },
]
...
const lineCount = computed(() => {
  const code =
    activeTab.value === 'html' ? htmlCode.value : activeTab.value === 'css' ? cssCode.value : jsCode.value
  return code.split('\n').length
})
...
watch(activeTab, () => {
  nextTick(() => {
    const ta =
      activeTab.value === 'html' ? htmlRef.value : activeTab.value === 'css' ? cssRef.value : jsRef.value
...
function handleTab(event: KeyboardEvent) {
  ...
  const codeRef = activeTab.value === 'html' ? htmlCode : activeTab.value === 'css' ? cssCode : jsCode
...
<template>
  <div class="flex h-dvh flex-col bg-bg-deep text-text-primary font-body overflow-hidden">
    <!-- ══════════════════════════════════════════════════════════════════════
         ...
         ══════════════════════════════════════════════════════════════════════ -->
    <header class="titlebar flex items-center border-b border-border-default bg-bg-surface shrink-0 select-none">
      <!-- Left: VS Code icon + menus -->
      <div class="flex items-center gap-0.5 pl-2">
        <!-- VS Code-style logo -->
        <div class="flex items-center justify-center size-7">
          <svg class="size-4" viewBox="0 0 24 24" fill="none">
            <path d="M17.58 2L8.77 10.28 4.15 6.77 2 7.87v8.26l2.15 1.1 4.62-3.51L17.58 22 22 20.17V3.83L17.58 2zM4 14.55V9.45L6.6 12 4 14.55zm13.58 4.58l-7.24-5.58L17.58 8v11.13z" fill="#38BDF8"/>
          </svg>
        </div>
      ...
      <div class="flex items-center gap-1 pr-1">
        ...
        <div class="hidden lg:flex items-center ml-2 gap-2">
          <button class="titlebar-btn hover:bg-bg-elevated">
            <svg class="size-3.5" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12" /></svg>
          </button>
          <button class="titlebar-btn hover:bg-bg-elevated">
            <svg class="size-3.5" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-width="2"><rect x="5" y="5" width="14" height="14" rx="0" /></svg>
          </button>
          <button class="titlebar-btn hover:bg-[#c42b1c]">
            <svg class="size-3.5" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><line x1="6" y1="6" x2="18" y2="18" /><line x1="18" y1="6" x2="6" y2="18" /></svg>
          </button>
        </div>
      </div>
    </header>
    ...
    <div class="flex flex-1 min-h-0 overflow-hidden">
      ...
      <nav class="hidden lg:flex flex-col items-center shrink-0 w-12 bg-bg-surface border-r border-border-default">
        <div class="flex flex-col items-center w-full pt-0.5 gap-0.5 flex-1">
          ...
          >
            <svg class="size-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
      ...
      >
        ...
        <div class="flex items-center justify-between px-4 h-9 shrink-0">
          <span class="text-[11px] font-medium tracking-wider text-text-secondary uppercase">Explorer</span>
        ...
        <div class="border-t border-border-default">
          <button class="flex items-center gap-1 w-full px-2 py-1 text-[11px] font-semibold tracking-wide text-text-secondary uppercase hover:text-text-primary">
            <svg class="size-3" viewBox="0 0 12 12" fill="currentColor"><path d="M4 2l4 4-4 4V2z"/></svg>
            Open Editors
          </button>
          <div class="pl-4">
            <button
              ...
              :class="activeTab === tab.id ? 'text-text-primary bg-bg-elevated' : 'text-text-secondary'"
        ...
        <div class="border-t border-border-default mt-1">
          <button class="flex items-center gap-1 w-full px-2 py-1 text-[11px] font-semibold tracking-wide text-text-secondary uppercase hover:text-text-primary">
            <svg class="size-3" viewBox="0 0 12 12" fill="currentColor"><path d="M2 3l4 4-4 4V3z" transform="rotate(90 6 6)"/></svg>
            Mini-Editor
          </button>
          <div class="pl-2">
            <!-- src folder -->
            <div class="flex items-center gap-1.5 px-2 py-0.5 text-[12px] text-text-secondary">
              <svg class="size-3.5 text-accent-amber/70 shrink-0" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div class="pl-4">
              <button
                ...
                :class="activeTab === tab.id ? 'text-text-primary bg-bg-elevated/50' : 'text-text-secondary'"
                @click="activeTab = tab.id"
              >
                <span class="size-1.5 rounded-full shrink-0" :class="tab.bgClass" />
                {{ tab.label }}
      ...
      <div class="flex flex-1 min-h-0 min-w-0 flex-col lg:flex-row">
        ...
        >
          ...
          <div class="flex lg:hidden border-b border-border-default bg-bg-surface overflow-x-auto">
            ...
            >
              <span class="size-1.5 rounded-full shrink-0" :class="tab.bgClass" />
              {{ tab.label }}
              <span v-if="activeTab === tab.id" class="absolute bottom-0 inset-x-0 h-px bg-accent-coral" />
            </button>
          </div>

          <!-- Breadcrumbs -->
          <div class="hidden lg:flex items-center gap-1 px-3 h-[22px] shrink-0 bg-bg-deep border-b border-border-default text-[11px] text-text-dim">
            <span>src</span>
            <svg class="size-3 text-text-dim/50" viewBox="0 0 24 24" fill="currentColor"><path d="M8 4l8 8-8 8"/></svg>
          ...
          <div class="flex flex-1 min-h-0 overflow-hidden">
            ...
            <div class="hidden xl:block w-[60px] shrink-0 bg-bg-deep border-l border-border-default/30 overflow-hidden">
              <div class="mt-4 mx-1.5 space-y-0.5 opacity-20">
                <div v-for="n in 25" :key="'mm-' + n" class="h-[2px] rounded-full" :class="n % 3 === 0 ? 'bg-accent-coral/40 w-3/4' : n % 5 === 0 ? 'bg-accent-sky/30 w-1/2' : 'bg-text-dim/40'" :style="{ width: (20 + Math.sin(n) * 40 + 30) + '%' }" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="hidden lg:block w-px bg-border-default shrink-0 cursor-col-resize hover:bg-accent-sky/50 transition-colors" />
        ...
        >
          <!-- Preview Tab Bar -->
          <div class="flex items-center justify-between border-b border-border-default bg-bg-surface shrink-0">
            <div class="flex items-center">
              <div class="flex items-center gap-2 px-3 h-[35px] text-xs text-text-secondary border-b border-text-secondary">
                <svg class="size-3.5 text-accent-sky" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="flex items-center gap-1.5 pr-3">
              <span class="size-2.5 rounded-full bg-accent-coral/30 border border-accent-coral/20" />
              <span class="size-2.5 rounded-full bg-accent-amber/30 border border-accent-amber/20" />
              <span class="size-2.5 rounded-full bg-green-400/30 border border-green-400/20" />
            </div>
          </div>

          <!-- URL Bar (decorative) -->
          <div class="hidden lg:flex items-center gap-2 px-3 py-1 bg-bg-deep border-b border-border-default shrink-0">
            <div class="flex items-center gap-1 text-text-dim">
              <svg class="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6" /></svg>
              <svg class="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6" /></svg>
              <svg class="size-3 ml-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M1 20V14a4 4 0 014-4h14l4-4"/></svg>
            </div>
            <div class="flex-1 flex items-center gap-1.5 bg-bg-surface/50 border border-border-default px-2 py-0.5 text-[11px] text-text-dim">
              <svg class="size-2.5 text-green-400/60 shrink-0" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10"/></svg>
    ...
    <!-- ══════════════════════════════════════════════════════════════════════
         ...
         ══════════════════════════════════════════════════════════════════════ -->
    <footer class="statusbar flex items-center justify-between border-t border-border-default shrink-0 select-none">
      <!-- Left cluster -->
      <div class="flex items-center h-full">
        <!-- Remote indicator -->
        <div class="flex items-center gap-1 h-full px-2 bg-accent-sky/15 text-accent-sky text-[11px]">
          <svg class="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 18 22 12 16 6" /><polyline points="8 6 2 12 8 18" /></svg>
        </div>
        <!-- Branch -->
        ...
        <!-- Errors / Warnings -->
        <button class="statusbar-item">
          <svg class="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" y2="15" /><line x1="9" y1="9" x2="15" y2="15" /></svg>
          0
          <svg class="size-3.5 ml-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
          0
        </button>
      </div>
      ...
      <div class="flex items-center h-full">
        ...
        <button class="statusbar-item" :class="activeTabInfo?.colorClass">{{ languageLabel }}</button>
        <!-- Notifications -->
